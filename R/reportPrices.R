...
reportPrices <- function(gdx,gdx_ref=NULL,output=NULL,regionSubsetList=NULL) {
  if(is.null(output)){
  ...
  febal.m        <- readGDX(gdx,name=c("q_balFe","q_febal"),types="equations",field="m",format="first_found",restore_zeros=FALSE)[,,fety]
  ...
  ####### read prices from reference runs to fill parameters for fixed years #########
  # if(!is.null(gdx_ref)) {
  #   if (file.exists(gdx_ref)) {
  #     cm_startyear    <- readGDX(gdx,name=c("cm_startyear"),format="first_found")
  #     pm_taxCO2eq_ref <- readGDX(gdx_ref,name=c("pm_taxCO2eq","pm_tau_CO2_tax"),format="first_found")
  #   }
  # }
  ...
  # if(!is.null(pm_taxCO2eq_ref)) {
  #   # replace values in pm_taxCO2eq to show reference CO2 tax values for fixed years
  #   pm_taxCO2eq[,y[y<cm_startyear],] <- pm_taxCO2eq_ref[,y[y<cm_startyear],]
  # }
