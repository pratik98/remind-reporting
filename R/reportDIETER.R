...
gdxToQuitte_hourly <- function(gdxfile){
  ...
  #idx <- as.numeric(str_extract(file, "[0-9]+"))
  #run_number = 1
...
#for the standalone mif, use the lig = "Lignite", and hc = "Hard coal", and not coal
dieter.tech.mapping.standalone <- c(dieter.tech.mapping,
                                    lig = "Lignite",
                                    hc = "Hard coal",
                                    NULL)
# dataframe you can do  revalue.levels(technology = dieter.tech.mapping) %>%  to above mapping
...
annual_new$Variable <- paste0(annual_new$Variable,"|",annual_new$Tech)
...
annual_remind <- annual_remind %>% filter(Model == "DIETER")
annual_remind <- annual_remind %>% revalue.levels(Tech = dieter.tech.mapping.REMIND)


annual_remind$Variable <- paste0(annual_remind$Variable,"|",annual_remind$Tech)
...
hourly_new$Variable <- paste0(hourly_new$Variable,"|",hourly_new$Tech)
